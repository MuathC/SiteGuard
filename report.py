...
# Load the model
model = YOLO("fire_HOPE.pt")
app = typer.Typer()
model.export(format='onnx', imgsz=640)
# # Define classes
# CLASSES = [
#     "boots", "gloves", "goggles", "helmet", "no-helm",
#     "no_glove", "no_goggles", "no_helmet", "no_shoes", "person", "vest","Person"
# ]
...
# # ---------- CONFIG ----------
# # If you have a homography (image->meters), set it here (3x3 np.array). Else keep as None for pixel mode.
# H = None  # e.g., np.array([[...],[...],[...]], dtype=np.float32)

# # Distance thresholds
# PROX_WARN_M   = 8.0     # meters (warn)  if H is not None
# PROX_CRIT_M   = 5.0     # meters (crit)  if H is not None
# PROX_COOLDOWN = 5.0     # seconds between alerts for the same person (to avoid spam)

# # Fallback pixel thresholds (if H is None)
# # Will be set dynamically based on frame size inside the loop (see code below)
# PROX_WARN_PX_RATIO = 0.12   # 12% of min(frame_w, frame_h)
# PROX_CRIT_PX_RATIO = 0.07   # 7%  of min(frame_w, frame_h)

# # ---------- HELPERS ----------
# def bottom_center(xyxy):
#     """Return bottom-center point (x,y) of a bbox [x1,y1,x2,y2]."""
#     x1, y1, x2, y2 = map(float, xyxy)
#     return ( (x1 + x2) * 0.5, y2 )

# def warp_points_H(points_xy, H):
#     """
#     Perspective-transform a list of (x,y) image points to ground-plane coordinates using homography H.
#     Returns Nx2 array of (X,Y) in 'meters' if H is built that way.
#     """
...
#     critical_incidents = sum(1 for v in tracker.person_safety_violations if
#                              "helmet" in v["details"].lower() or "vest" in v["details"].lower())
...
#     # Track last alert time per "person key" to avoid spamming
#     # We'll use a coarse key made from the person's bottom-center (quantized)
#     tracker.prox_last_alert = {}  # { person_key: last_alert_unix_time }
...
#         # Gather boxes
#         person_boxes = current_frame_detections.get("Person", [])  # exact class name from your mapping
#         vehicle_boxes = list(current_frame_detections.get("vehicle", [])) + \
#                         list(current_frame_detections.get("machinery", []))

#         # Nothing to do if we don't see both
#         if len(person_boxes) > 0 and len(vehicle_boxes) > 0:
#             # Compute bottom-centers in image space
#             p_pts_img = [bottom_center(b) for b in person_boxes]
#             v_pts_img = [bottom_center(b) for b in vehicle_boxes]

#             # Decide thresholds
#             if H is None:
#                 # Pixel mode
#                 px_min_dim = min(width, height)
#                 warn_thr = PROX_WARN_PX_RATIO * px_min_dim
#                 crit_thr = PROX_CRIT_PX_RATIO * px_min_dim

#                 # Use image points directly
#                 p_pts_plane = p_pts_img
#                 v_pts_plane = v_pts_img
#                 units = "px"
#             else:
#                 # Meter mode
#                 p_pts_plane = warp_points_H(p_pts_img, H)  # -> (X,Y) in meters
#                 v_pts_plane = warp_points_H(v_pts_img, H)
#                 warn_thr = PROX_WARN_M
#                 crit_thr = PROX_CRIT_M
#                 units = "m"

#             # For drawing & logging
#             now_unix = time.time()
#             # We'll also keep a list of lines to draw (person_pt_img -> nearest_vehicle_pt_img)
#             prox_lines = []  # list of (p_img_xy, v_img_xy, severity, distance, nearest_idx)

#             for i, P_plane in enumerate(p_pts_plane):
#                 # Find nearest vehicle to this person
#                 nearest_j = None
#                 nearest_d = None
#                 for j, V_plane in enumerate(v_pts_plane):
#                     d = euclid(P_plane, V_plane)
#                     if (nearest_d is None) or (d < nearest_d):
#                         nearest_d = d
#                         nearest_j = j

#                 # Hysteresis / cooldown per person
#                 # Build a coarse person key from their image bottom-center (quantized to 10px)
#                 p_img_xy = p_pts_img[i]
#                 person_key = (int(p_img_xy[0] // 10), int(p_img_xy[1] // 10))

#                 severity = None
#                 if nearest_d is not None:
#                     if nearest_d < crit_thr:
#                         severity = "CRITICAL"
#                     elif nearest_d < warn_thr:
#                         severity = "WARNING"

#                 if severity is not None:
#                     last_t = tracker.prox_last_alert.get(person_key, 0)
#                     if now_unix - last_t >= PROX_COOLDOWN:
#                         # Log an event
#                         tracker.person_safety_violations.append({
#                             "timestamp": current_time,
#                             "event": "Worker–Vehicle Proximity",
#                             "details": f"{severity}: distance={nearest_d:.2f}{units}"
#                         })
#                         tracker.prox_last_alert[person_key] = now_unix

#                     # For visualization: keep the nearest line info
#                     v_img_xy = v_pts_img[nearest_j]
#                     prox_lines.append((p_img_xy, v_img_xy, severity, nearest_d, nearest_j))

#             # Draw proximity lines (green/yellow/red)
#             for (p_xy, v_xy, sev, dist_val, _) in prox_lines:
#                 color = (0, 255, 0)  # green
#                 if sev == "WARNING":
...
#                 p = (int(p_xy[0]), int(p_xy[1]))
#                 v = (int(v_xy[0]), int(v_xy[1]))
#                 cv2.line(frame, p, v, color, 2)
#                 cv2.circle(frame, p, 4, color, -1)
#                 cv2.circle(frame, v, 4, color, -1)
#                 label = f"{sev} {dist_val:.1f}{units}"
#                 cv2.putText(frame, label, (p[0] + 6, p[1] - 6), cv2.FONT_HERSHEY_SIMPLEX, 0.55, color, 2, cv2.LINE_AA)
#         # -------------------------------------------------------------
